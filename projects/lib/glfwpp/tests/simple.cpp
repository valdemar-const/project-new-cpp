...
print_gl_info()
{
    ...
    {
        bool is_supported = extension_supported(ext_name);
        if (is_supported)
        {
            ++supported_count;
            std::cout << "\t"s + std::string {ext_name} << std::endl;
        }
    }
    std::cout << "\textensions_count: " << supported_count << std::endl;
}
...
)
{
    ...
    const std::vector<Window_Hint> hints {
    ...
    auto window = create_window(640, 640, "Wrapped GLFW", nullptr, nullptr);
    make_context_current(window);
    swap_interval(1);

    assert(gladLoadGLLoader((GLADloadproc)get_proc_address));
